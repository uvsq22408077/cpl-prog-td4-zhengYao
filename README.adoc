= _Gestionnaire de Fichiers en Commandes en Ligne avec Annotations _
...
* L'application permettra à l'utilisateur d'annoter un _ER_
...
* L'interface proposera une visualisation du répertoire courant en mode texte en quatre parties. La partie:
  ...
  . affiche les _ER_ du répertoire avec les _NoER_
...
* Toutes les notes associées aux éléments du répertoire courant seront stockés dans un seul fichier appelé "notes.txt" dans le répertoire courant.
